(() => {
  ...
  function findReelsConnection(root) {
    ...
    function visit(value, depth) {
      ...
      if (Array.isArray(value.edges)) {
        const reels = value.edges
          .map((edge) => parseMedia(mediaNodeFromEdge(edge)))
          .filter((reel) => reel && reel.code);
        if (reels.length && (!best || reels.length > best.reels.length)) {
          best = { connection: value, reels };
        }
      }
  ...
  async function requestGraphqlPage(query, cursor, count) {
    ...
    const found = findReelsConnection(data);
    const connection = found?.connection || data?.data?.[query.rootField];
    const edges = Array.isArray(connection?.edges) ? connection.edges : [];
    const reels = found?.reels || edges
      .map((edge) => parseMedia(mediaNodeFromEdge(edge)))
      .filter((reel) => reel && reel.code);
    return {
      reels,
  ...
  function mediaFromSeebal(item) {
    ...
    return {
      ...
      media_type: 2,
      product_type: "clips",
      play_count: safeNumber(item.viewCount),
      video_view_count: safeNumber(item.viewCount),
      like_count: safeNumber(item.likeCount),
      comment_count: safeNumber(item.commentCount),
      taken_at: safeNumber(item.takenAt),
      video_duration: safeNumber(item.duration),
      owner_id: safeString(item.author?.id),
      username: safeString(item.author?.username),
      caption: safeString(item.caption),
      thumbnail_url: safeString(item.thumbnailUrl),
      video_url: safeString(item.videoUrl),
      is_verified: false,
      location_name: "",
      coauthor_producers: null,
      profile_pic_url: safeString(item.author?.avatar),
      follower_count: null,
      outlier_score: null,
      fetched_at: Date.now()
    };
  ...
  function fetchSeebalFeedPage() {
    return new Promise((resolve, reject) => {
      const requestId = `feed-${Date.now()}-${Math.random().toString(16).slice(2)}`;
      const timer = setTimeout(() => {
        window.removeEventListener("message", onMessage);
        reject(new Error("SEEBAL feed timeout"));
      }, 45000);
      function onMessage(event) {
        const data = event.data || {};
        if (data.type !== "SEEBAL_FEED_RESPONSE" || data.requestId !== requestId) return;
        clearTimeout(timer);
        window.removeEventListener("message", onMessage);
      ...
      window.postMessage({ type: "SEEBAL_FEED_REQUEST", requestId, cursor: state.nextCursor || "" }, "*");
    });
  }
  ...
  async function loadReels({ more = false } = {}) {
    ...
    try {
      ...
      if (!media.length) {
        try {
          const seebalPage = await fetchSeebalFeedPage();
          const seebalItems = Array.isArray(seebalPage.items) ? seebalPage.items.map(mediaFromSeebal).filter(Boolean) : [];
          media = seebalItems.filter((reel) => !state.fetchedCodes.has(reel.code));
          media.forEach((reel) => state.fetchedCodes.add(reel.code));
          state.nextCursor = seebalPage.cursor || null;
        } catch (fallbackError) {
          console.warn("[Local Reels Analyzer] SEEBAL feed fallback failed", fallbackError);
        }
      }
